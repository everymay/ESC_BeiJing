...
void InitBeforeReadROM(void)
{
	...
	StateFlag.onceTimeAdcAutoAdjust = 1; //置位ADC零偏校准标志位。WY
	...
	/*初始化三相状态机。WY*/
	StateEventFlag_A = STATE_EVENT_STANDBY_A; //初始化A相状态机。WY
	StateEventFlag_B = STATE_EVENT_STANDBY_B; //初始化B相状态机。WY
	StateEventFlag_C = STATE_EVENT_STANDBY_C; //初始化C相状态机。WY
	...
	/*上电时，继电器等待操作。WY*/
	ESCFlagA.RELAYCONTROLFlag = 1; //A相小继电器处于等待操作状态。WY
	ESCFlagB.RELAYCONTROLFlag = 1; //B相小继电器处于等待操作状态。WY
	ESCFlagC.RELAYCONTROLFlag = 1; //C相小继电器处于等待操作状态。WY
	...
	ESCBYRelayCNTA = 0; //A相旁路磁保持继电器状态正常。WY
	ESCBYRelayCNTB = 0; //B相旁路磁保持继电器状态正常。WY
	ESCBYRelayCNTC = 0; //C相旁路磁保持继电器状态正常。WY

	ESCFlagA.PHASE = 1; //A相。WY
	ESCFlagB.PHASE = 2; //B相。WY
	ESCFlagC.PHASE = 3; //C相。WY

	SET_POWER_CTRL(1); //15V电源上电。WY
	Delayus(TIME_WRITE_15VOLT_REDAY); //延时10ms。WY
...
/*
 * 功能：计时。每5ms，递增计数器的值。WY
 */
void clock5ms(void)
{
	static int j = 2; //循环变量。WY
	int16 *pCnt; //指向计数器的指针。WY
	Uint16 i; //循环变量。WY

	pCnt = (int16*) &CntMs;
	for (i = sizeof(CntMs); i != 0; i--) //遍历结构体。WY
	{
		if ((*pCnt)++ > 30000) //递增计数器的值，且计数器的值上溢。WY
		{
			(*pCnt) = 30000; //赋值计数器的值。30000 * 5ms = 150s。WY
		}
		pCnt++; //指向下一个结构体成员。WY
	}

	pCnt = (int16*) &ESCFlagA.ESCCntMs;
	for (i = sizeof(ESCFlagA.ESCCntMs); i != 0; i--) //遍历结构体。WY
	{
		if ((*pCnt)++ > 30000) //递增计数器的值，且计数器的值上溢。WY
		{
			(*pCnt) = 30000; //赋值计数器的值。30000 * 5ms = 150s。WY
	...
	for (i = sizeof(ESCFlagB.ESCCntMs); i != 0; i--) //遍历结构体。WY
	{
		if ((*pCnt)++ > 30000) //递增计数器的值，且计数器的值上溢。WY
		{
			(*pCnt) = 30000; //赋值计数器的值。30000 * 5ms = 150s。WY
	...
	for (i = sizeof(ESCFlagC.ESCCntMs); i != 0; i--) //遍历结构体。WY
	{
		if ((*pCnt)++ > 30000) //递增计数器的值，且计数器的值上溢。WY
		{
			(*pCnt) = 30000; //赋值计数器的值。30000 * 5ms = 150s。WY
	...
	MainContactConfirm(25); //此函数为空函数，无意义。WY
}

/*
 * 功能：计时。每1s，递增计数器的值。WY
 */
