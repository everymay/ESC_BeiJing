...
void FirstFaultStop(Uint16 ESCPHASE)
{
			...
			&& (ESCPHASE == 1)) //A相。WY
	{
		...
		ESCFlagA.FAULTCONFIRFlag = 0; //复位A相故障信号标志位。WY
...
{
	if (GET_BYPASS_CONTACT_ACTION_A == 0) //A相旁路磁保持继电器闭合。WY
	{
		if (++BypassContactCntA >= TESTVALAUE)
		{// 检测点数必须多一点,要不然可能会出现负载断流的情况
			BypassContactCntA = TESTVALAUE;
			ESCFlagA.ByPassContactFlag = 1; //A相旁路处于通路状态。WY
		}
	}
	else //A相磁保持继电器断开
	{
	...
	if (GET_MAIN_CONTACT_ACTION_A == 0) //检测到A相主路磁保持继电器闭合信号。WY
	{
		if (++ContactorFeedBackCntA >= TESTVALAUE) //连续检测到A相主路磁保持继电器闭合信号。WY
		{
			ContactorFeedBackCntA = TESTVALAUE;
			ESCFlagA.ContactorFeedBackFlag = 1; //A相主路磁保持继电器闭合。WY
		}
	}
	else //检测到A相主路磁保持继电器断开信号。WY
	{
		if (--ContactorFeedBackCntA <= 0)  //连续检测到A相主路磁保持继电器断开信号。WY
		{
			ContactorFeedBackCntA = 0;
			ESCFlagA.ContactorFeedBackFlag = 0; //A相主路磁保持继电器断开。WY
		}
	}
...
void SetFaultDelayCounter(Uint16 counterName, Uint16 mode)
{
	...
	else if (CntFaultDelay[counterName] > mode)
	{
		CntFaultDelay[counterName] -= mode; //计数器减去指定值。
	}
	else
	{
		CntFaultDelay[counterName] = 0;
