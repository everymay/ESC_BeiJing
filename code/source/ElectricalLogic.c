...
void ESCSTANDBYEVENT(ESCCTRLVALFLAG *ESCFlag)
{
	...
	if (ESCFlag->PHASE == 1)
	{
		...
		SET_IGBT_ENA(IGBT_DISABLE); //失能PWM。WY
		DisablePWMA(); //封锁PWM。WY

		/*
		 * 以下函数功能存疑。理由如下：WY
		 * GPIO79始终为输出模式。通过读取该引脚电平状态来反馈磁保持继电器状态无意义。
		 */
		if (GET_MAIN_CONTACT_ACTION_A == ESC_ACTION_ENABLE) //主路磁保持继电器闭合。WY
		{
			SET_MAIN_CONTACT_ACTION_A(ESC_ACTION_DISABLE); //断开主路磁保持继电器。WY
		}
	}
	...
	if (ESCFlag->PHASE == 2)
	{
		SET_BYPASS_CONTACT_ACTION_B(ESC_ACTION_ENABLE); //闭合旁路磁保持继电器。WY
		SET_IGBT_ENB(IGBT_DISABLE); //失能PWM。WY
		DisablePWMB(); //封锁PWM。WY

		if (GET_MAIN_CONTACT_ACTION_B == ESC_ACTION_ENABLE) //主路磁保持继电器闭合。WY
		{
			SET_MAIN_CONTACT_ACTION_B(ESC_ACTION_DISABLE); //断开主路磁保持继电器。WY
		}
	}
	...
	if (ESCFlag->PHASE == 3)
	{
		SET_BYPASS_CONTACT_ACTION_C(ESC_ACTION_ENABLE); //闭合旁路磁保持继电器。WY
		SET_IGBT_ENC(IGBT_DISABLE); //失能PWM。WY
		DisablePWMC(); //封锁PWM。WY

		if (GET_MAIN_CONTACT_ACTION_C == ESC_ACTION_ENABLE) //主路磁保持继电器闭合。WY
		{
			SET_MAIN_CONTACT_ACTION_C(ESC_ACTION_DISABLE); //断开主路磁保持继电器。WY
		}
	}
	...
	if (ESCFlag->onceTimeStateMachine == 1)
	{
		...
		if (ESCFlag->ByPassContactFlag == 1) //旁路磁保持继电器闭合。WY
		{
			...
			if (ESCFlag->PHASE == 3)
			{
				SET_MAIN_CONTACT_ACTION_C(ESC_ACTION_DISABLE); //断开A相主路磁保持继电器。WY
		...
		{
			SetFaultDelayCounter(CNT_BYPASS_NOCLOSE_A, 0); //清空【A相旁路磁保持继电器闭合故障】的次数。WY
		}

		/*处理B相*/
		if ((ESCFlag->ESCCntMs.ContactorDelay >= CNT_MS(5000)) && (StateEventFlag_B == STATE_EVENT_STANDBY_B))
		{//5S之内,旁路磁保持必须合闸,反馈必须传送过来,否则会报旁路合闸故障
		...
		{
			SetFaultDelayCounter(CNT_BYPASS_NOCLOSE_C, 0); //清零故障信号计数器。WY
		}
	}
}

/*
 * 功能：执行【预充电】状态。WY。预充电的作用是？
 * 输入参数：待执行的X相参数。X的取值集合为{A, B, C}。
 */
	...
	if (ESCFlag->ESCCntSec.ChargingTime >= CNT_SEC(3)) //充电时长满足条件。WY
	{
		if (++ESCFlag->EscWaitRunDelayCnt >= 300) //计时单位是？？？作用是？？WY
		{
					...
					|| (ESCFlag->DelayFlag == 1))) //使得
			{
				...
				/*
				 * 由于：1. 磁保持继电器动作需要时间；2. 对磁保持继电器的状态检测作连续采样处理；
				   故在发送闭合A相主路磁保持继电器的指令后，无法立即检测到A相主路磁保持继电器处于闭合状态。

				 * 若A相主路磁保持继电器存在故障。
				 * 由于状态机不存在转向【故障停机】的入口，可能导致状态机卡死在【预充电】状态？
				 */
				if (ESCFlag->ContactorFeedBackFlag == 1) //A相主路磁保持继电器闭合。该标志位在AD-ISR中被修改。WY
				{
					...
					ESCFlag->ESCCntSec.HWPowerSupDelay = 0; //为执行A相待机状态做好准备。WY
				}
			}

			/*处理B相*/
			if ((ESCFlag->PHASE == 2) && ((GET_MAIN_CONTACT_ACTION_B == ESC_ACTION_DISABLE) || (ESCFlag->DelayFlag == 1)))//磁保持继电器开通逻辑是低电平开通;
			{
				ESCFlag->EscWaitRunDelayCnt = 300;
				ESCFlag->DelayFlag = 1;
				SET_MAIN_CONTACT_ACTION_B(ESC_ACTION_ENABLE);//B相 高\低压磁保持继电器合闸
				if (ESCFlag->ContactorFeedBackFlag == 1)
				{
					ESCFlag->DelayFlag = 0;
					StateEventFlag_B = STATE_EVENT_WAIT_B;//状态机进入WAIT状态
					ESCFlag->ESCCntSec.S_RSwiTiDelay = 0;
			...
			if ((ESCFlag->PHASE == 3) && ((GET_MAIN_CONTACT_ACTION_C == ESC_ACTION_DISABLE) || (ESCFlag->DelayFlag == 1)))//磁保持继电器开通逻辑是低电平开通;
			{
				ESCFlag->EscWaitRunDelayCnt = 300;
				ESCFlag->DelayFlag = 1;
				SET_MAIN_CONTACT_ACTION_C(ESC_ACTION_ENABLE);//C相 高\低压磁保持继电器合闸
				if (ESCFlag->ContactorFeedBackFlag == 1)
				{
					ESCFlag->DelayFlag = 0;
					StateEventFlag_C = STATE_EVENT_WAIT_C;//状态机进入WAIT状态
					ESCFlag->ESCCntSec.S_RSwiTiDelay = 0;
					ESCFlag->ESCCntSec.HWPowerSupDelay = 0;
				}
			}
...
{
	if ((ESCFlag->ESCCntMs.PllDelay > CNT_MS(1000)) //为锁相环锁相分配的时长大于设定值。WY
			&& (ESCFlag->onceTimeStateMachine == 0)) //单次状态检测已完成。WY
	{
		switch (StateFlag.VoltageMode)
		{
			...
			{
				/*无需升压*/
				if ((ESCFlag->VoltIn_rms > ESCFlag->StableValue) //电网电压有效值 > 稳定值，无需升压？。WY
						|| (ESCFlag->gridCur_rms < LoadCurlimit)) //电网电流有效值 < 负载电流限定值，存在故障？WY
				{//输入电压高于稳定值,或负载电流小于设定值,置为IGBT直通状态
					if (++ESCFlag->EscStandyDelayCnt1 > ESC_STANDY_DELAY_TIME_POINT) //持续100个状态机周期？具体时长是多少？。WY
					{
						...
						if (ESCFlag->PHASE == 2)
						{
							ESCFlag->PWM_ins_index = 1; //B相主路直通。WY
						}

						/*处理C相*/
						if (ESCFlag->PHASE == 3)
						{
							ESCFlag->PWM_ins_index = 1; //C相主路直通。WY
						}
					}
				}
				else /*需要升压。WY*/
				...
				/*需要升压，且满足升压条件。WY*/
				if ((ESCFlag->VoltIn_rms < ESCFlag->StableValue)  //电网电压有效值 < 稳定值，需要升压。WY
						&& (ESCFlag->gridCur_rms >= LoadCurlimit)) //电网电流有效值 > 负载电流限定值，满足升压条件。WY
			...
			case 1://降压
			{
				...
				if ((ESCFlag->VoltIn_rms > ESCFlag->StableValue) && (ESCFlag->gridCur_rms >= LoadCurlimit))
				{//输入电压高于门限值,且负载电流大于设定值+2,置位buck状态
 ...
 * 输入参数：待执行的X相参数。X的取值集合为{A, B, C}。
 *
 * 说明：当设备工作在升压模式，若当前无需升压，则进入运行转待机状态。
 */
void ESCRUNTURNWAIT(ESCCTRLVALFLAG *ESCFlag)
{
//三相电压均高于稳定值(稳定值=门限值+回差值),且均处于直通状态一段时间后
	switch (StateFlag.VoltageMode)
	{
	case 0://升压
	case 4:
	{
		...
		{
			if (ESCFlag->ESCCntSec.R_SSwiTiDelay >= CNT_SEC(R_SSwitchTime)) //持续时长超过设定值。WY
			{//该时间需要设置长一点,防止第一版电操频繁投切.
				...
				if (ESCFlag->PHASE == 2)
				{
					SET_SCRB_ENABLE(ESC_ACTION_ENABLE); //导通B相旁路晶闸管。WY
					SET_BYPASS_CONTACT_ACTION_B(ESC_ACTION_ENABLE); //闭合B相旁路磁保持继电器。WY
					SET_IGBT_ENB(IGBT_DISABLE); //失能B相PWM。WY
					DisablePWMB(); //封锁B相PWM。WY
					ESCFlag->StateSkipFlag = 1;
					StateEventFlag_B = STATE_EVENT_WAIT_B; //切换状态机。状态切换至：B相待机状态。WY
					ESCFlag->ESCCntSec.HWPowerSupDelay = 0;
				}
				...
				if (ESCFlag->PHASE == 3)
				{
					SET_SCRC_ENABLE(ESC_ACTION_ENABLE); //导通B相旁路晶闸管。WY
					SET_BYPASS_CONTACT_ACTION_C(ESC_ACTION_ENABLE); //闭合B相旁路磁保持继电器。WY
					SET_IGBT_ENC(IGBT_DISABLE); //失能B相PWM。WY
					DisablePWMC(); //封锁A相PWM。WY
					ESCFlag->StateSkipFlag = 1;
					StateEventFlag_C = STATE_EVENT_WAIT_C; //切换状态机。状态切换至：C相待机状态。WY
					ESCFlag->ESCCntSec.HWPowerSupDelay = 0;
				}
	...
	case 1: //降压。WY
		if ((ESCFlag->VoltIn_rms < ESCFlag->StableValue) && (ESCFlag->PWM_ins_index == 1))//单相
		{
			...
			{//该时间需要设置长一点,防止第一版电操频繁投切.
				...
				if (ESCFlag->PHASE == 1)
				{
					SET_SCRA_ENABLE(ESC_ACTION_ENABLE); //导通A相旁路晶闸管。WY
					SET_BYPASS_CONTACT_ACTION_A(ESC_ACTION_ENABLE); //闭合A相旁路磁保持继电器。WY
					SET_IGBT_ENA(IGBT_DISABLE); //失能A相PWM。WY
					DisablePWMA(); //封锁A相PWM。WY
					ESCFlag->StateSkipFlag = 1;
					StateEventFlag_A = STATE_EVENT_WAIT_A; //切换状态机。状态切换至：A相待机状态。WY
					ESCFlag->ESCCntSec.HWPowerSupDelay = 0;
				...
				if (ESCFlag->PHASE == 3)
				{
					SET_SCRC_ENABLE(ESC_ACTION_ENABLE); //导通C相旁路晶闸管。WY
					SET_BYPASS_CONTACT_ACTION_C(ESC_ACTION_ENABLE); //闭合C相旁路磁保持继电器。WY
					SET_IGBT_ENC(IGBT_DISABLE); //失能C相PWM。WY
					DisablePWMC(); //封锁C相PWM。WY
					ESCFlag->StateSkipFlag = 1;
					StateEventFlag_C = STATE_EVENT_WAIT_C; //切换状态机。状态切换至：C相待机状态。WY
					ESCFlag->ESCCntSec.HWPowerSupDelay = 0;
...
void ESCWAITTURNRUN(ESCCTRLVALFLAG *ESCFlag)
{
	...
	{
		case 0:	//升压模式。WY
		case 3: //VoltageModeFlag的值只能为0或1.此处存疑。WY
		case 4: //VoltageModeFlag的值只能为0或1.此处存疑。WY
		{
			/*需要升压。WY*/
					...
					&& (ESCFlag->gridCurrBY_rms < RateLoadCurrLimit)) //旁路电流有效值 < 负载额定电流限定值。WY
			{
				...
				{//任意一相的欠压持续5000个点//约4S
					...
					if (ESCFlag->PHASE == 1)
					{
						...
						Delayus(TIME_WRITE_15VOLT_REDAY); //延时2ms。WY
						EnablePWMA(); //解锁A相PWM。WY
						PIVolA.i10 = 0;//清理饱和
						PIVolA.i6 = 0;
						Delayus(100);//需要加个延时时间,否则PWM的寄存器会来不及动作。与影子寄存器有关？WY
						SET_IGBT_ENA(IGBT_ENABLE); //使能A相PWM。WY
						SET_SCRA_ENABLE(ESC_ACTION_ENABLE); //开通A相旁路晶闸管。为何要导通晶闸管？WY
					...
					if (ESCFlag->PHASE == 2)
					{
						...
						EnablePWMB(); //解锁B相PWM。WY
						PIVolB.i10 = 0;//清理饱和
						PIVolB.i6 = 0;
						Delayus(100);//需要加个延时时间,否则PWM的寄存器会来不及动作
						SET_IGBT_ENB(IGBT_ENABLE); //使能B相PWM。WY
						SET_SCRB_ENABLE(ESC_ACTION_ENABLE);//开晶闸管
						SET_BYPASS_CONTACT_ACTION_B(ESC_ACTION_DISABLE); //断开B相旁路磁保持继电器。WY
						StateEventFlag_B = STATE_EVENT_RUN_B; //切换状态机。切换状态至：B相正常运行.WY
					...
					if (ESCFlag->PHASE == 3)
					{
						...
						ESCFlag->PWM_ins_index = 1;//IGBT直通态
						Delayus(TIME_WRITE_15VOLT_REDAY);
						EnablePWMC(); //解锁C相PWM。WY
						PIVolC.i10 = 0;//清理饱和
						PIVolC.i6 = 0;
						Delayus(100);//需要加个延时时间,否则PWM的寄存器会来不及动作
						SET_IGBT_ENC(IGBT_ENABLE); //使能C相PWM。WY
						SET_SCRC_ENABLE(ESC_ACTION_ENABLE);//开晶闸管
						SET_BYPASS_CONTACT_ACTION_C(ESC_ACTION_DISABLE); //断开C相旁路磁保持继电器。WY
						StateEventFlag_C = STATE_EVENT_RUN_C; //切换状态机。切换状态至：C相正常运行.WY
			...
			{
				if ((ESCFlagA.ESCCntSec.HWPowerSupDelay >= CNT_SEC(TIME_STANDBY_DELAY)) //15V电源开启时长超过600s。WY
						&& (ESCFlagB.ESCCntSec.HWPowerSupDelay >= CNT_SEC(TIME_STANDBY_DELAY))
				...
				{
					ESCFlagA.ESCCntSec.S_RSwiTiDelay = 0; //电网欠压时长为0。WY
				}

				if (ESCFlag->PHASE == 2) //处理B相。WY
				{
					ESCFlagB.ESCCntSec.S_RSwiTiDelay = 0; //电网欠压时长为0。WY
				}

				if (ESCFlag->PHASE == 3) //处理C相。WY
				{
					ESCFlagC.ESCCntSec.S_RSwiTiDelay = 0; //电网欠压时长为0。WY
				}
			}
		}
		break;
		...
		case 1:
		{
			...
			{
				SET_POWER_CTRL(1);//给硬件供15V电源,硬件是否需要延时???
				Delayus(TIME_WRITE_15VOLT_REDAY);
				ESCFlagA.ESCCntSec.HWPowerSupDelay = 0;
				ESCFlagB.ESCCntSec.HWPowerSupDelay = 0;
				ESCFlagC.ESCCntSec.HWPowerSupDelay = 0;
			...
			else //未满足降压模式开启条件。WY
			{
				if ((ESCFlagA.ESCCntSec.HWPowerSupDelay >= CNT_SEC(TIME_STANDBY_DELAY)) && (ESCFlagB.ESCCntSec.HWPowerSupDelay >= CNT_SEC(TIME_STANDBY_DELAY))
						&& (ESCFlagC.ESCCntSec.HWPowerSupDelay >= CNT_SEC(TIME_STANDBY_DELAY)))
				{
					if ((StateEventFlag_A == STATE_EVENT_WAIT_A) && (StateEventFlag_B == STATE_EVENT_WAIT_B) && (StateEventFlag_C == STATE_EVENT_WAIT_C)) //三相均处于待机状态。WY
					{
...
void ESCSTOPSTATE(ESCCTRLVALFLAG *ESCFlag)
{
	...
	if (ESCFlag->PHASE == 2)
	{
		SET_SCRB_ENABLE(ESC_ACTION_ENABLE); //开通B相旁路晶闸管。WY
		SET_BYPASS_CONTACT_ACTION_B(ESC_ACTION_ENABLE); //闭合B相旁路磁保持继电器。WY
		SET_IGBT_ENB(IGBT_DISABLE); //失能B相PWM。WY
		DisablePWMB(); //封锁B相PWM。WY
	}
	...
	if (ESCFlag->PHASE == 3)
	{
		SET_SCRC_ENABLE(ESC_ACTION_ENABLE); //开通C相旁路晶闸管。WY
		SET_BYPASS_CONTACT_ACTION_C(ESC_ACTION_ENABLE); //闭合C相旁路磁保持继电器。WY
		SET_IGBT_ENC(IGBT_DISABLE); //失能C相PWM。WY
		DisablePWMC(); //封锁C相PWM。WY
	}
	...
	ESCFlag->ESCCntMs.ContactorDelay = 0; //为执行【待机】状态做好准备。WY
}
...
{
	if ((ESCFlag->ESCCntSec.PRECHARGEDelayBY >= CNT_SEC(2)) && (ESCFlag->ESCCntSec.PRECHARGEDelay >= CNT_SEC(2)))
	{//上电两秒之后小继电器在动作;
		if (ESCFlag->RELAYCONTROLFlag == 1) //需要操作继电器。WY
		{
					...
					&& (ESCFlag->ContactorFeedBackFlag == 0)) //主路磁保持继电器断开。WY
			{
				...
				if (ESCFlag->PHASE == 2) //处理B相。WY
				{
					SET_GV_VOL_CTRL_B(1); //闭合B相小继电器。WY
				}

				if (ESCFlag->PHASE == 3) //处理C相。WY
				{
					SET_GV_VOL_CTRL_C(1); //闭合C相小继电器。WY
				}
...
//上电1秒晶闸管不通.然后就要一直导通
//只要旁路合,晶闸管就合.
//只要旁路分.等待三相反馈触点后,晶闸管分.
int UnitSUCCESSCNT = 0;
int UnitDISSUCCESSCNT = 0;
int DelayFlagA = 0,DelayFlagB = 0,DelayFlagC = 0;
...
void SetStartCtrl(void)
{
	...
	while (1)
	{
		...
		if (STOP_INSTRUCTION == 0) //停机按钮被按下。WY
		{
			...
			{
				if ((!ESCFlagA.startFlag) && (!ESCFlagB.startFlag) && (!ESCFlagC.startFlag)) //三相均处于未启动状态。WY
				{
					SoeRecData(SOE_GP_EVENT); //生成操作日志。WY
				}

				ESCFlagA.stopFlag = 0; //A相未停机。WY
				ESCFlagB.stopFlag = 0; //B相未停机。WY
				ESCFlagC.stopFlag = 0; //C相未停机。WY

				ESCFlagA.startFlag = 1; //A相启动。WY
				ESCFlagB.startFlag = 1; //B相启动。WY
				ESCFlagC.startFlag = 1; //C相启动。WY
				...
				SET_POWER_CTRL(1); //15V电源上电。WY
				Delayus(TIME_WRITE_15VOLT_REDAY); //延时。WY
				SET_RUNNING_LED(0); //设备运行指示灯灭。

				if ((StateEventFlag_A != STATE_EVENT_FAULT_A) && (StateEventFlag_B != STATE_EVENT_FAULT_B)
						&& (StateEventFlag_C != STATE_EVENT_FAULT_C)) //三相均未处于故障停机状态。WY
				{
					SET_FAULT_LED(0); //设备故障指示灯灭。WY
				}
			}
		...
		{
			/*停止设备。WY*/
			if ((CntMs.inputStop >= CNT_MS(360)) && (CntMs.inputStop <= CNT_MS(2000))) //停止设备的条件：停机按钮被按下的时长在(0.36s, 2s)之间 。WY
			{
				if ((!ESCFlagA.stopFlag) && (!ESCFlagB.stopFlag) && (!ESCFlagC.stopFlag)) //三相均处于未停机状态。WY
				{
					SoeRecData(SOE_GP_EVENT + 1); //生成操作日志。WY
				}

				ESCFlagA.stopFlag = 1; //A相停机。WY
				ESCFlagB.stopFlag = 1; //B相停机。WY
				ESCFlagC.stopFlag = 1; //C相停机。WY

				ESCFlagA.startFlag = 0; //A相未启动。WY
				ESCFlagB.startFlag = 0; //A相未启动。WY
				ESCFlagC.startFlag = 0; //A相未启动。WY
		...
		{
			/*A相初始状态（正常停机）。WY*/
			case STATE_EVENT_STANDBY_A://初始状态(正常停机)。WY
			{
				ESCSTANDBYEVENT(&ESCFlagA); //执行初始状态(正常停机)。WY
				ESCFlagA.stopFlag = 0; //A相处于未停机状态。WY

				if (ESCFlagA.startFlag == 1) //A相启动。WY
				{
					SoeRecData(SOE_GP_EVENT); //生成操作日志。
					ESCFlagA.stopFlag = 0; //A相处于未停机状态。WY

					if ((GET_GV_VOL_CTRL_A == 1) //A相小继电器闭合。WY
							&& (GET_BYPASS_FEEDBACK == 1) //微型断路器断开。WY
							&& (ESCBYRelayCNTA != 1) //A相旁路磁保持继电器状态正常。WY
							&& (ESCSicFaultCNTA != 1)) //A相SiC管状态正常。WY
					{
						StateEventFlag_A = STATE_EVENT_RECHARGE_A; //切换状态机至A相充电。WY
						ESCFlagA.BYFEEDBACKFLAG = 1; //旁路状态正常。WY
					}

					ESCFlagA.ESCCntSec.ChargingTime = 0; //清零预充电时间。为进入预充电状态做准备。WY
				}
			}
			break;

			/*A相预充电。WY*/
			case STATE_EVENT_RECHARGE_A://软起充电
			{
			...
			/*A相运行*/
			case STATE_EVENT_RUN_A://调制运行 --- 当收到停机指令或者故障时，才进行状态跳转
			{
				if (ESCFlagA.onceTimeStateMachine == 1) //单次状态检测。WY
				{
					...
					{//只要旁路分.等待三相反馈触点后,晶闸管分.
						SET_SCRA_ENABLE(ESC_ACTION_DISABLE); //关断A相旁路晶闸管。为啥？WY
...
//三相电压均高于稳定值(稳定值=门限值+回差值),且均处于直通状态一段时间后
				...
				ESCRUNTURNWAIT(&ESCFlagA); //运行转待机？WY

				SET_RUNNING_LED(1); //设备运行指示灯亮。WY

				/*紧急停机。WY*/
				if (ESCFlagA.stopFlag == 1)//执行停机状态的渐渐关PWM前的动作
				{
					ESCFlagA.stopFlag = 0;
					ESCFlagA.startFlag = 0;
					ESCFlagA.PeakStopFlag = 1;
					SET_SCRA_ENABLE(ESC_ACTION_ENABLE);//只要旁路合,晶闸管就合
					StateEventFlag_A = STATE_EVENT_STOP_A;//停机逻辑
				}
			}
			break;

			/*执行A相停机状态。WY*/
			case STATE_EVENT_STOP_A://--- 停机软关 --- 逐步停止电流输出，最终封锁PWM，跳转进入状态1
			{
				ESCSTOPSTATE(&ESCFlagA);
				if (GET_BYPASS_FEEDBACK != 0)
				{
					SET_RUNNING_LED(0);
				}
#if !TEST_RUNPORCESS //未定义？WY
			...
			/*执行A相待机状态。WY*/
			case STATE_EVENT_WAIT_A://小于电压设定值且无故障
			{
				/*执行A相待机状态*/
			...
			case STATE_EVENT_FAULT_A:
				ESCFlagA.startFlag = 0; //设备未启动。WY
				ESCFlagA.stopFlag = 1; //设备停机。WY
				SET_RUNNING_LED(0);
				FAULTSTATE(&ESCFlagA); //执行故障状态。WY
				if (ESCBYRelayCNTA != 1)
				{
...
// ****************B相状态机*******************//
		...
		{
		case STATE_EVENT_STANDBY_B://初始状态(正常停机)
...
//保持磁保持继电器分闸状态
		...
		case STATE_EVENT_RECHARGE_B://软起充电
			SET_RUNNING_LED(1);//亮
...
//                ESCFlagB.PWMcurrDirFlag = 0;                //debuggg
		...
		case STATE_EVENT_RUN_B://调制运行 --- 当收到停机指令或者故障时，才进行状态跳转
			if (ESCFlagB.onceTimeStateMachine == 1)
			{
				...
				{//只要旁路分.等待三相反馈触点后,晶闸管分.
					SET_SCRB_ENABLE(ESC_ACTION_DISABLE);
					ESCFlagB.onceTimeStateMachine = 0;//讨论:这里加拒分检测?,拒合很重要
				}
			}
			ESCRUNEVENT(&ESCFlagB);//B相
//三相电压均高于稳定值(稳定值=门限值+回差值),且均处于直通状态一段时间后
			ESCRUNTURNWAIT(&ESCFlagB);
			SET_RUNNING_LED(1);
			if (ESCFlagB.stopFlag == 1)//执行停机状态的渐渐关PWM前的动作
			{
				ESCFlagB.stopFlag = 0;
				ESCFlagB.startFlag = 0;
//                ESCFlagB.PWMcurrDirFlag = 0;                //debuggg
				...
				StateEventFlag_B = STATE_EVENT_STOP_B;//停机逻辑
			}
			break;

		case STATE_EVENT_STOP_B://--- 停机软关 --- 逐步停止电流输出，最终封锁PWM，跳转进入状态1
...
#if !TEST_RUNPORCESS
			ExecuteOpenContact_B();//IGBT接触器分闸
#endif
			StateEventFlag_B = STATE_EVENT_STANDBY_B;
			SoeRecData(SOE_GP_EVENT + 1);//soe中有阻塞代码,故放在IO操作之后
//            ESCFlagB.HWPowerSupFlag = 0;
			ESCFlagB.ESCCntSec.HWPowerStopDelay = 0;
			ESCFlagB.ESCCntSec.HWPowerFaultDelay = 0;
			break;

		case STATE_EVENT_WAIT_B://小于电压设定值且无故障
		...
		case STATE_EVENT_FAULT_B:// --- 故障停机 ---
			...
			if (ESCBYRelayCNTB != 1)
			{
				ExecuteOpenContact_B();//断开磁保持继电器
			}
//输出指示灯信号：运行指示灯灭，故障指示灯亮
			SET_FAULT_LED(1);
			break;
		default:
			break;
...
// *****************C相状态机***************//
		...
		{
		case STATE_EVENT_STANDBY_C://初始状态(正常停机)
...
//保持磁保持继电器分闸状态
		...
		case STATE_EVENT_RECHARGE_C://软起充电
			SET_RUNNING_LED(1);//亮
...
//                ESCFlagC.PWMcurrDirFlag = 0;                //debuggg
		...
		case STATE_EVENT_RUN_C://调制运行 --- 当收到停机指令或者故障时，才进行状态跳转
			if (ESCFlagC.onceTimeStateMachine == 1)
			{
				...
				{//只要旁路分.等待三相反馈触点后,晶闸管分.
					SET_SCRC_ENABLE(ESC_ACTION_DISABLE);
					ESCFlagC.onceTimeStateMachine = 0;//讨论:这里加拒分检测?,拒合很重要
				}
			}
			ESCRUNEVENT(&ESCFlagC);//C相
//三相电压均高于稳定值(稳定值=门限值+回差值),且均处于直通状态一段时间后
			ESCRUNTURNWAIT(&ESCFlagC);
			SET_RUNNING_LED(1);
			if (ESCFlagC.stopFlag == 1)//执行停机状态的渐渐关PWM前的动作
			{
				ESCFlagC.stopFlag = 0;
				ESCFlagC.startFlag = 0;
//                ESCFlagC.PWMcurrDirFlag = 0;                //debuggg
		...
		case STATE_EVENT_STOP_C://--- 停机软关 --- 逐步停止电流输出，最终封锁PWM，跳转进入状态1
...
#if !TEST_RUNPORCESS
			ExecuteOpenContact_C();//IGBT接触器分闸
#endif
...
//输出指示灯信号：运行指示灯灭，故障指示灯亮
		...
		if ((ESCFlagA.resetFlag == 1) && (StateEventFlag_A == STATE_EVENT_FAULT_A))// 软件检测故障，而非硬件反馈故障，可排查后手动复位
		{
			StateEventFlag_A = STATE_EVENT_STOP_A;
