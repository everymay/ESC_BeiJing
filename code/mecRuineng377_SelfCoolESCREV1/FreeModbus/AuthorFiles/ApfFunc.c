...
eMBErrorCode DefineReadWriteRegister(UCHAR *pucRegBuffer,USHORT usAddress,USHORT usNRegs,eMBRegisterMode eMode)
{
	...
	switch(eMode)
	{
		...
		case MB_REG_READ:
		{
			if((usAddress >= 0x81FC) && (usAddress <= 0x81FE))
			{   //地址自动匹配
			...
			if(usAddress >= 0x81FF)
				pZdy = usAddress - 0x81FF + ((int*) &MuFaciID);   //单元总数

			/*读取厂家设置参数。WY*/
			if((usAddress >= 0x8200) && (usAddress <= 0x83DF))
			{
				pZdy = usAddress - 0x8200 + ((int*) &FactorySet);
			}
			...
			{
				pZdy = usAddress - 0x8400 + ((int*) &UserSetting);
			}
			...
			if((usAddress >= 0xF9F8) && (usAddress <= 0xF9F9))
			{
				...
				if(StateFlag.SOEFullFlag)
					SOERead.ChannelNumber = RECORD_NUMBER;
				else
					SOERead.ChannelNumber = SOE.CurrentPointerPosition;
			}
		...
		case MB_REG_WRITE:
		{
			...
			if((usAddress >= 0x83E0) && (usAddress <= 0x83FF))
			{     //零偏校准下发
				pZdy = (int16*) &VariZeroOffsetVAL;
			...
			if((usAddress >= 0x8000) && (usAddress <= 0x81FB))
			{     //遥控区
				rx34 = *pucRegBuffer ++ << 8;
				rx34 |= *pucRegBuffer;
...
void ElseWrite(Uint16 ADDR, int16 DATA)
{
	switch(ADDR)
	{
		...
		case 0x8001:
			if(DATA == 1)
			{
				if((!ESCFlagA.startFlag) && (!ESCFlagB.startFlag) && (!ESCFlagC.startFlag))
               ...
               ESCFlagC.startFlag = 1;
#endif
				if((StateEventFlag_A == STATE_EVENT_STANDBY_A) || (StateEventFlag_B == STATE_EVENT_STANDBY_B) || (StateEventFlag_C == STATE_EVENT_STANDBY_C))
				{
					SET_POWER_CTRL(1);
		...
		case 0x8002:
			if(DATA == 1)
			{
				if((!ESCFlagA.stopFlag) && (!ESCFlagB.stopFlag) && (!ESCFlagC.stopFlag))
		...
		case 0x8008:
			if(DATA < 31)
				StateFlag.RespondFlag = DATA;			//蓝牙连接后闪烁从机指示灯
		break;
		case 0x8009:												//界面波形显示刷新
			if((DATA == 1) && (ReFFTDelayTim >= 30))
			{												//故障录波后的30s内不允许FFT模式
		...
		case 0x800D:
			if((DATA == 0x5A)
					&& (((StateEventFlag_A == STATE_EVENT_STANDBY_A) || (StateEventFlag_A == STATE_EVENT_FAULT_A))
		...
		case 0x800E:												//相序、谐波校正
			if(DATA % 2)
				StateFlag.SequenceAutoFlag = 1;
			StateFlag.PowerFactor = (DATA & 0xFF00) >> 8;
		break;
		case 0x800F:												//故障录波擦除

			RecordFlash.UiBlockRead = DATA & 0xFF;
			if(!RecordFlash.UiBlockRead)
				RecordFlash.FlashBlockRead = RecordFlash.RecordBlockNum - 1;
			else
				RecordFlash.FlashBlockRead = RecordFlash.UiBlockRead - 1;
			FlashDataRead();

			if((((DATA >> 8) & 0x01) == 1)
					&& ((StateEventFlag_A == STATE_EVENT_STANDBY_A) || (StateEventFlag_B == STATE_EVENT_STANDBY_B)
		...
		case 0x8015:
			if(DATA == 1)
				StateFlag.onceTimeAdcAutoAdjust = 1;	//零偏校准刷新
		break;
		case 0x8016:
			if((DATA & 0x01) == 1)
				RecordFlash.FlashState.B.RemoteControlEn = 1;	//远程控制位
			else
				RecordFlash.FlashState.B.RemoteControlEn = 0;	//远程控制位
		break;
		case 0x8017:
			if(DATA == 0x5A)
				StateFlag.FanctrlTestFlag = 1;
		break;
		default:
		break;
	}
