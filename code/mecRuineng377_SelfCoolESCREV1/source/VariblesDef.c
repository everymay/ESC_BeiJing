...
//#pragma DATA_SECTION(PWM_ins_indexA,            "CLADataLS01");
//#pragma DATA_SECTION(PWM_ins_indexB,            "CLADataLS01");
//#pragma DATA_SECTION(PWM_ins_indexC,            "CLADataLS01");
...
float TIMER1 = 0,TIMER2 = 0,TIMER3 = 0,TIMER4 = 0,TIMERPERIODVAL1 = 0,TIMERPERIODVAL2 = 0,PERIODVAL1 = 0,PERIODVAL2 = 0;

int CAPTIEMRFlag = 0;

float ESC_DutyDataA = 1.0f,ESC_DutyDataB = 1.0f,ESC_DutyDataC = 1.0f;

float OUTCUR_OVER_INS = 300; //电流瞬时值上限。WY	 // peak : 144.342*1.414 * 1.3 = 265

float OUTCUR_OVER_INS_NEG = -300; //电流瞬时值下限。WY

float OUTCUR_OVER_RMS = 174; //电流有效值过流上限。WY        // RMS  : 115*1.2 = 173
float OUTCUR_FAULTCONTACT = 30;

float OUT_OVER_LOAD = 130; //电流有效值过载上限。WY    // RMS  : 115*1.1 = 158
...
float GridBPCurrAF = 0; //A相旁路电流瞬时值？WY
float GridBPCurrBF = 0; //B相旁路电流瞬时值？WY
float GridBPCurrCF = 0; //C相旁路电流瞬时值？WY
...
float VoltInAF = 0; //A相电网电压（瞬时值）WY
float VoltInBF = 0; //B相电网电压（瞬时值）WY
float VoltInCF = 0; //C相电网电压（瞬时值）WY
...
float GridCurrAF = 0; //A相电网电流瞬时值？WY
float GridCurrBF = 0; //B相电网电流瞬时值？WY
float GridCurrCF = 0; //C相电网电流瞬时值？WY
...
int ArithFlagA = 0; //A相电网电流突变标志位。0，存在突变；1，不存在突变。WY
int ArithFlagB = 0; //B相电网电流突变标志位。0，存在突变；1，不存在突变。WY
int ArithFlagC = 0; //C相电网电流突变标志位。0，存在突变；1，不存在突变。WY
...
float VoltInA_rms = 0; //A相电网电压有效值。WY
float VoltInB_rms = 0; //B相电网电压有效值。WY
float VoltInC_rms = 0; //C相电网电压有效值。WY

float VoltOutA_rms = 0; //A相负载电压有效值。WY
float VoltOutB_rms = 0; //B相负载电压有效值。WY
float VoltOutC_rms = 0; //C相负载电压有效值。WY
...
float gridCurA_rms = 0; //A相电网电流有效值。WY
float gridCurB_rms = 0; //B相电网电流有效值。WY
float gridCurC_rms = 0; //C相电网电流有效值。WY

float gridCurrBYAF_rms = 0; //A相旁路电流有效值。WY
float gridCurrBYBF_rms = 0; //B相旁路电流有效值。WY
float gridCurrBYCF_rms = 0; //C相旁路电流有效值。WY
...
int CTRL24_POWERFlag = 0; //15V电源状态标志位。0：已执行响应；1：等待执行响应。WY

Uint16 windColdCtr; //故障屏蔽标志位。0，屏蔽该故障；1，不屏蔽该故障。WY
...
volatile int StateEventFlag_A = STATE_EVENT_STANDBY_A; //A相状态机。初始状态为：初始状态（正常停机）。WY
volatile int StateEventFlag_B = STATE_EVENT_STANDBY_B; //B相状态机。初始状态为：初始状态（正常停机）。WY
volatile int StateEventFlag_C = STATE_EVENT_STANDBY_C; //C相状态机。初始状态为：初始状态（正常停机）。WY
...
int ESCSicFaultCNTA = 0; //A相SiC管状态。0，正常；1，异常。WY
int ESCSicFaultCNTB = 0; //B相SiC管状态。0，正常；1，异常。WY
int ESCSicFaultCNTC = 0; //C相SiC管状态。0，正常；1，异常。WY
...
float ArithVAL=3; //电网电压突变值上限。WY
float ArithVal=3; //电网电压突变值下限。WY
...
float ESCDCVOLITLIMIT = 800; //电容电压上限值。WY   // 380*1.414*1.15 + 100 = 718
...
/*
 * 三相不平衡度。WY
 * 计算公式：三相不平衡度 = （三相电流中有效值的最大者 - 三相电流有效值的均值） / 三相电流有效值的均值
 */
...
float PIVAL = 3.141593f; //π的浮点数近似值。WY
...
//#pragma DATA_SECTION(ErrorRecord,"EBSS3");
//#pragma DATA_SECTION(MuFaciID,"EBSS3");
//#pragma DATA_SECTION(UserSetting,"EBSS3");
#pragma DATA_SECTION(FactorySet,"EBSS2");
//#pragma DATA_SECTION(VariZeroOffset,"EBSS3");
